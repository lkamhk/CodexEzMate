...
Windows 上的 Codex 常驻助手：查看剩余用量、管理本机会话、代管 App Server，以及在额度恢复后继续指定的 Goal。

**当前源代码版本：1.21.0** · Windows x64 · .NET 8 / WPF · 繁体中文／简体中文／English
...
| Goal 监控 | 通过 App Server 后台恢复已勾选、因额度耗尽而暂停的原生 Goal，无需前台点击。 |
...
> **实验功能：** Goal 监控尚未经开发者本人实际使用验证，稳定性及兼容性仍待确认。请自行评估是否启用；建议先以非重要工作测试，并留意执行结果。

1. 开启「Goal 监控」，扫描本机对话。
2. 勾选允许自动恢复的目标，再开始监控。
3. 原生 Goal 处于 `usageLimited`，且执行工作的 App Server 确认额度已恢复时，程序会逐个后台接手。

支持同一 Windows 账户下、CLI／Codex App／VS Code 来源的兼容本机已保存对话。内部子会话、ephemeral 对话及远程／WSL 工作目录不在支持范围；需要原客户端专属工具的工作可能无法接手。请勿同时在另一客户端继续已被接手的对话。

需要命令、文件修改或权限批准时，可在「查看／回复请求」处理，程序不会自动批准。停止监控只停止接手新工作；要停止当前工作，请使用「暂停当前工作」。人工暂停的 Goal 不会被正常自动检查重新启动。
