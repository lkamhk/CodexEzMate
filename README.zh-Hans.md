...
> **Goal 监控仍在开发中，请勿使用，并保持关闭。** 目前尚未可靠支持在原来的 Codex Desktop 对话中自动恢复工作。此警告仅针对 Goal 监控功能。
...
| Goal 监控 | **开发中，请勿使用。** Goal 自动恢复尚未可靠运行。 |
...
> **仍在开发中，请勿使用。** 请保持 Goal 监控关闭，直到后续版本明确宣布可以使用。

目前尚未可靠支持在原来的 Codex Desktop 对话中自动恢复工作，执行进度的显示也有待完善；CLI 及 VS Code 的兼容性仍未完成验证。请勿依赖此功能在额度重置后继续工作，也不要为了强制接手而关闭 Codex Desktop。

如已启用，请点击「停止监控」，取消程序启动后持续监控的选项，然后保存。停止监控只会停止接手新工作；若 EzMate 正在执行 Goal，请先点击「暂停当前工作」。

此警告仅针对 Goal 监控，无需因此关闭用量显示、Session Browser 或 Server 代管。
