...
> **Goal 監控仍在開發中，請勿使用，並保持關閉。** 目前尚未可靠支援在原本的 Codex Desktop 對話中自動恢復工作。此警告只針對 Goal 監控功能。
...
| Goal 監控 | **開發中，請勿使用。** Goal 自動恢復尚未可靠運作。 |
...
> **仍在開發中，請勿使用。** 請保持 Goal 監控關閉，直至後續版本明確宣佈已可使用。

目前尚未可靠支援在原本的 Codex Desktop 對話中自動恢復工作，執行進度的顯示亦待完善；CLI 及 VS Code 的相容性仍未完成驗證。請勿依賴此功能在額度重置後繼續工作，亦不要為了強制接手而關閉 Codex Desktop。

如已啟用，請按「停止監控」，取消「程式啟動後持續監控」，然後儲存。停止監控只會停止接手新工作；若 EzMate 正在執行 Goal，請先按「暫停目前工作」。

此警告只針對 Goal 監控，毋須因此關閉用量顯示、Session Browser 或 Server 代管。
